...
# To readers: reading guidelines  
# To compute standardised incidence and mortality rates, several datasets are needed : 
#      - one dataset containing individual incident cases with age at incidence, gender, year of incidence, type of cancer (ICD10). This dataset is the result of the merging of 2 other datasets extracted from the registry database (table a_patient that contains gender and a_tumor that contains age at incidence, year of incidence, type of cancer (ICD10))
#      - one dataset containing individual death cases  with age at death, gender, year of death, type of cancer (ICD10), 
#      - one dataset with the Geneva population at risk by gender, age and year, 
#      - one dataset with the reference population (Waterhouse et al., 1976)


# The beginning of the programm will load and clean those dataframes. Individual incidence and death data will be aggregated to obtain the number of incident cases and number of death by cancer type and gender.
# They will be combined in a final dataframe that will contain the number incident,death cases, at-risk population for each stratum of age and sex, for each tumor type. Age will be considered in 5 years categories going from 0 to 85 and older. Gender is coded 1 for male and 2 for women. The period analysed is 2016-2020.

# Because all these data manipulation can easily lead to errors, numerous checks are performed all along the data wrangling.

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## Packages ----
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Some of these packages were loaded when trying to solve some issues but are not finaly used
# Tex packages are left because the final version of this document (outside Graph project) will be pdf
...
tumor_raw <- sqlQuery(conn, a_tumor) %>% 
 ...
 tumor<- tumor_raw %>% 
  ...
  filter(behavior=="C") %>%
  filter(icd10!="C44") %>% 
  dplyr::select(pid, tid, age, year, icd10)  # Explicitly call select() from dplyr
...
cities_db <- "select id as INCIDENCE_CITY_ID, NPA, GNR, EXP, DATE_FROM, DATE_TO, TEXT_FR, text_en  from cities"
...
labels <- c("0 to 4 years", "5 to 9 years", "10 to 14 years", "15 to 19 years", "20 to 24 years", "25 to 29 years", "30 to 34 years", "35 to 39 years", "40 to 44 years", "45 to 49 years", "50 to 54 years", "55 to 59 years", "60 to 64 years", "65 to 69 years", "70 to 74 years", "75 to 79 years", "80 to 84 years", "85 and older")
